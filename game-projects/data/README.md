...
移动榜单历史通过 AppMagic 的 App Store / Google Play 游戏免费榜和畅销榜采集，覆盖中国大陆、香港、台湾、日本、韩国、东南亚（新加坡样本）与美国；中国大陆仅采集 iOS。每次快照保存当地日期、小时与时区：榜内产品保存精确名次；未出现在完整 Top 100 的目标产品只保存“未入 Top 100”状态，不得写成第 101 名。公开逐小时端点只提供当前约一周的日期窗口，因此只回填来源实际返回的历史日期，并从此按日累积；更早日期不得用当前名次反推。AppMagic 榜单名次目前不参与跨平台表现等级。
...
Phase 29 的 JORO 修正属于用户指出线索后的定向核验，不建立发行商白名单，也不穷举发行商产品目录。历史遗漏改按 IP 维度处理：从已知日本 IP、系列官网、商店标题别名和游戏资料库发现候选，再回到游戏官网、官方商店、版权方或发行公告核验。日常自动任务继续负责近期新闻、预约页、计划日期到期状态和已收录产品表现；历史长尾采用分批 IP 回溯，避免大型发行商的原创、代理和地区发行项目造成无界噪音。
