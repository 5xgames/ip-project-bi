...
Phase 29 起增加“发行商产品履历反查”。系统维护发行商官网的 SERVICE / WORKS / PRODUCTS 目录清单，每日比较官网产品链接、标题别名、项目 ID 与数据库记录，并把未匹配条目写入独立核验队列。该层专门补足已经停运、商店下架且近期没有新闻或榜单记录的历史项目；官网目录只负责发现，仍须以游戏官网、官方商店、版权方或发行商公告核验上线时间、停运时间和平台后才能入库。JORO 为首个接入目录，审计补入《Code Geass Genesic Re;CODE》与《欢迎来到实力至上主义教室～合并拼图特别考试～》；《Coin Musme》《AniPoko》因未识别为外部日本娱乐 IP 游戏化而保留排除理由。
